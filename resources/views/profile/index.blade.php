    ...
    <div class="mt-5 grid grid-cols-4 gap-2">
        @foreach ($sekolah as $item)
            <div class="max-w-sm p-6 bg-white border border-gray-200 rounded-lg shadow-sm">
                ...
                <a href="{{ route('profile.show',$item->uuid) }}" class="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm w-full sm:w-auto px-5 py-2.5 text-center cursor-pointer">Lihat Profile</a>
            </div>
        @endforeach
    </div>
</div>
